...
<task type="auto">
  ...
  <action>
Create end-to-end user journey tests that exercise the full MVC stack. These tests simulate complete shopping flows through multiple "pages" (re-rendering DOM fixtures to simulate page navigation in this MPA architecture).
...
2. `describe('Guest User Shopping Flow')` -- MVC-09, MVC-10
   ...
   SINGLE comprehensive journey test as requested by user:
   ...
   g. **Verify ALL prices updated to ILS**: Item price shows '₪' and '185'. Cart total recalculated to ILS. Quantity PRESERVED at 1 (not reset).
...
3. `describe('Logged-In User Shopping Flow')` -- MVC-09, MVC-10
...
4. `describe('Currency change mid-checkout consistency')` -- MVC-09
...
5. `describe('Navigation with unsaved cart changes')` -- MVC-10
...
**beforeEach pattern for each describe block:**
```javascript
beforeEach(() => {
  resetFactoryCounter();
  model.cart.length = 0;
...
```
  ...
  <verify>`cd frontend && npx vitest run tests/integration/user-journeys.test.js --reporter=verbose` -- all tests pass with 0 failures</verify>
  <done>user-journeys.test.js has 12+ passing tests. Complete guest shopping journey verified end-to-end. Logged-in user API path verified. Currency mid-checkout consistency verified with round-trip (USD->ILS->USD). Cart persistence across page navigations verified. Requirements MVC-09 and MVC-10 verified.</done>
</task>

<task type="auto">
  <name>Task 2: Final verification and test count</name>
  <files></files>
  <action>
