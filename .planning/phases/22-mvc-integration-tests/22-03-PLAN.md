...
```
  ...
  <verify>`cd frontend && npx vitest run tests/integration/lifecycle.test.js --reporter=verbose` -- all tests pass with 0 failures</verify>
  <done>lifecycle.test.js has 15+ passing tests. Mount initialization verified for menu, footer, flags, currency. Update lifecycle verified for content replacement. Cleanup verified: no duplicate handlers after rapid switching. Async lifecycle verified. Requirements MVC-06, MVC-07, MVC-08 verified.</done>
</task>
