...
- CartView's currency-changed event handler calls `this._render()` which doesn't exist (known bug from D20-01-02). Test currency switching via manual `render()` calls with different localStorage currency values
...
3. `describe('Language change propagation')` -- MVC-05
   ...
   - `it('should update ALL currency selector text when language switches')`:
     ...
     - setLanguage('heb'), verify currency options show 'דולר'/'שקל'
...
```
  ...
  <done>model-view-sync.test.js has 20+ passing tests. Currency switching verified for ALL price elements (not spot-checks). Language switching verified for menu, footer, currency selectors. Cart badge updates verified for guest and logged-in paths. Requirements MVC-03, MVC-04, MVC-05 verified.</done>
</task>
